...
fn agentComputer(p: vec2f) -> f32 {
  let q = rotate(p, -0.04);
  let bezel = sdRoundedBox(q, vec2f(0.14, 0.095), 0.018);
  let screen = sdRoundedBox(q, vec2f(0.118, 0.072), 0.01);
  let stand = sdRoundedBox(q - vec2f(0.0, 0.125), vec2f(0.038, 0.026), 0.008);
  let foot = sdRoundedBox(q - vec2f(0.0, 0.153), vec2f(0.085, 0.008), 0.004);
  var mark = opSmoothUnion(bezel, stand, 0.012);
  mark = opSmoothUnion(mark, foot, 0.006);
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let markA = agentComputer((p - vec2f(0.44, -0.20)) / breathe);
  let markB = agentComputer((p - vec2f(0.25, 0.08)) / breathe);
  let markC = agentComputer((p - vec2f(0.56, 0.22)) / breathe);
  let marks = min(markA, min(markB, markC));
  let fill = 1.0 - smoothstep(-0.0015, 0.004, marks);
  let line = 1.0 - smoothstep(0.0, 0.0055, abs(marks));
  let glow = exp(-max(marks, 0.0) * 16.0);
  ...
  let teal = vec3f(0.196078, 0.690196, 0.623529);
  let blue = vec3f(0.0, 0.356863, 0.560784);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(teal, blue, smoothstep(0.38, 0.82, uv.x));
  col = mix(col, paper, spark * 0.45 + line * 0.05);
